...
def print_discos_keys():
    ...
    with open(full_path_public, "r") as f:
        print(f.read())
    print(f"\nPath of the public key file: {full_path_public}")
    print(f"Remember to never share the '{key_filename}.key_secret' file with anyone.")
    print(
...
def main():
    ...
    parser.add_argument(
        "--show-only",
        action="store_true",
        help="Only prints the public key and its path without generating a new pair."
    )
